 ...
 *
 * - Uses sport IDs derived from Convex `sportGroupMappings` as the `dd_sports` filter.
...
export const sphereImport = action({
  ...
  handler: async (ctx, args) => {
    ...
    const [allSportIds, mappings] = await Promise.all([
      ctx.runQuery(api.services.sportGroupMappings.getAllSportIds, {}),
      ctx.runQuery(api.services.sportGroupMappings.listSportGroupMappings, {}),
    ]);

    if (allSportIds.length === 0) {
      throw new Error("No sport group mappings found. Run generateSportGroupMapping first.");
    }
    ...
    for (const sphereEntry of sphereEntries) {
      ...
      try {
        const mappedData = mapSphereToBlogPost(sphereEntry, lookup);
